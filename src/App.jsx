...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#120d08', color: '#c8a96e', fontFamily: "Georgia, 'Times New Roman', serif", paddingBottom: 60, fontSize: 16 }}>

      {/* ── Top bar ── */}
      <div style={{ background: '#0e0a06', borderBottom: '1px solid #2a1f14', padding: '18px 32px', display: 'flex', alignItems: 'center', gap: 20 }}>
        <div style={{ ...S.mono, fontSize: 16, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#6a5438', fontWeight: 700 }}>
          Painter's Notebook
        </div>
        <div style={{ color: '#3a2c1e', fontSize: 13, fontStyle: 'italic' }}>Warhammer 40,000</div>

        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 12 }}>
          {saveStatus !== 'idle' && <span style={{ fontSize: 12, ...S.mono, color: SAVE_COLORS[saveStatus] }}>{SAVE_LABELS[saveStatus]}</span>}
          {initError && <span style={{ fontSize: 12, ...S.mono, color: '#7a3828' }} title={initError}>⚠ DB offline — changes won't persist</span>}
          <button onClick={() => setPanel(panel === 'scan' ? null : 'scan')} style={{ ...S.mono, fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase', background: panel === 'scan' ? '#3a2c1e' : 'none', border: '1px solid #3a2c1e', borderRadius: 3, padding: '7px 16px', color: '#8a6848', cursor: 'pointer' }}>
            Scan Folder
          </button>
          <button onClick={() => setPanel(panel === 'manage' ? null : 'manage')} style={{ ...S.mono, fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase', background: panel === 'manage' ? '#3a2c1e' : 'none', border: '1px solid #3a2c1e', borderRadius: 3, padding: '7px 16px', color: '#8a6848', cursor: 'pointer' }}>
            Manage
          </button>
          <button onClick={() => setPanel(panel === 'intake' ? null : 'intake')} style={{ ...S.mono, fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase', background: '#2d5a27', border: 'none', borderRadius: 3, padding: '8px 18px', color: '#c8a96e', cursor: 'pointer' }}>
            + Add Entry
          </button>
        </div>
      </div>
      ...
      {/* ── Army tabs ── */}
      <div style={{ background: '#0e0a06', borderBottom: '1px solid #1e160e', padding: '0 32px', display: 'flex', gap: 0, overflowX: 'auto' }}>
        {notebook.armies.map((a) => (
          <button key={a.id} onClick={() => setActiveArmy(a.id)} style={{ background: 'none', border: 'none', borderBottom: activeArmy === a.id ? `2px solid ${a.accentLight}` : '2px solid transparent', padding: '14px 22px', color: activeArmy === a.id ? a.accentLight : '#4a3828', ...S.mono, fontSize: 14, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            {a.name}
          </button>
        ))}
        ...
        {showIndex
          ? <div>
              <div style={{ borderBottom: '2px solid #4a3828', marginBottom: 24, paddingBottom: 14 }}>
                <div style={{ ...S.mono, fontSize: 26, fontWeight: 700, color: '#c8a96e', letterSpacing: '0.05em', textTransform: 'uppercase' }}>By Part</div>
                <div style={{ color: '#5a4830', fontSize: 13, marginTop: 4, fontStyle: 'italic' }}>All recipes across all armies, grouped by model surface</div>
              </div>
